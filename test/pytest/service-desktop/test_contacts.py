...
def test_add_contact(harness):
    ...
    assert first_contact_record == retrieved_contact_record
    current_number_of_contact_records = initial_number_of_contact_records + 1

    """
    Attempt to add duplicate contact
    """
    assert contacts_tester.detect_adding_duplicated_contact(first_contact_record), "Failed to detect duplicate!"
    ...
    assert result, "Failed to get contact by ID!"
    del retrieved_contact_record["id"]
    assert second_contact_record == retrieved_contact_record
    ...
    assert result, "Failed to get contact by ID!"
    del retrieved_contact_record["id"]
    assert third_contact_record == retrieved_contact_record
    current_number_of_contact_records += 1
    ...
    assert result, "Failed to get contact by ID!"
    del retrieved_contact_record["id"]
    assert fourth_contact_record == retrieved_contact_record
    current_number_of_contact_records += 1
    ...
    result, received_contact_records_count = contacts_tester.get_contacts_count()
    assert result, "Failed to get contacts count!"
    assert current_number_of_contact_records == received_contact_records_count
    ...
    assert returned_contacts[0] == fourth_contact_record
    assert returned_contacts[1] == second_contact_record
    assert returned_contacts[2] == first_contact_record
    assert returned_contacts[3] == third_contact_record
    assert current_number_of_contact_records == total_count
    ...
    for record in returned_contacts:
        ...
        del record["id"]
    # Returned contacts are sorted by altName
    assert returned_contacts[0] == first_contact_record
    assert returned_contacts[1] == third_contact_record
    assert current_number_of_contact_records == total_count
    ...
    result, received_contact_records_count = contacts_tester.get_contacts_count()
    assert result, "Failed to get contacts count!"
    assert initial_number_of_contact_records == received_contact_records_count
